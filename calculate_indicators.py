        ...
        for code in stock_codes:
            ...
            try:
                ...
                if not industry_name:
                    logger.warning(f"業種名の取得に失敗しました: code={code}")
                    continue
                ...
                if not stock_data:
                    logger.warning(f"株価データの取得に失敗しました: code={code}")
                    continue
                
                # インジケーターを計算してDBに保存
                success = indicator_service.calculate_and_save_indicators(stock_data, code, industry_name)
                if success:
                    logger.info(f"インジケーター計算・保存成功: code={code}")
                else:
                    logger.warning(f"インジケーター計算・保存失敗: code={code}")
                    
            except Exception as e:
                logger.error(f"銘柄処理中のエラー: code={code}, error={str(e)}")
                continue
