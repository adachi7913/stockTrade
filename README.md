...
株式取引に関する情報は、以下の3つのテーブルで管理されています：

1. **entries テーブル**
   - 購入時点の情報を管理
   - エントリー価格、損切り価格、目標価格などの購入時の判断情報を保持

2. **holding_evaluations テーブル**
   - 保有中の株式の評価情報を管理
   - 定期的な保有判断（HOLD/SELL）、評価時点の株価、更新された損切り価格や目標価格を保持

3. **trade_results テーブル**
   - 手仕舞い（売却）時の情報を管理
   - 最終的な取引結果（損益、保有期間など）の履歴を保持
   - 資金残高の管理も担当
