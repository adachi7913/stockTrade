...
def filter_stock(stock_code, close, market_cap, last_no_entry_date=None, no_entry_span=None, volume_data=None, atr=None, rsi=None, stoch_k=None, min_close_threshold=300, close_threshold=3000, market_cap_threshold=1_000_000_000):
    ...
    if close < min_close_threshold:
        logger.info(f"{stock_code}: 終値 {close} 円 は最低価格 {min_close_threshold} 円以上の条件を満たしていません。")
        return False

    # 終値の上限チェック
    if close > close_threshold:
        logger.info(f"{stock_code}: 終値 {close} 円 は {close_threshold} 円以下の条件を満たしていません。")
        return False

    # 時価総額チェック
    if market_cap is None:
        logger.info(f"{stock_code}: 時価総額情報が取得できなかったため除外。")
        return False
    if market_cap < market_cap_threshold:
        logger.info(f"{stock_code}: 時価総額 {market_cap} 円 は {market_cap_threshold} 円以上の条件を満たしていません。")
        return False
