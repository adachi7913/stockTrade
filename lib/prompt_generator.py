...
"""
        ...
        prompt += f"""
## 判断指示
この銘柄へのエントリー（ロングまたはショート）が推奨されるか、もしくは見送るべき（ホールド）かを判断し、結果を**日本語で回答**してください。

【判断基準】
- 直近の技術指標（特にトレンドとモメンタム）
- バックテスト結果の勝率と平均リターン
- 事前スコア
- 現在の価格水準
...
- 下降トレンドが期待できるか？
- 売られすぎの兆候はないか？
- リスクリワード比が1.5以上見込めるか？（ショートの場合、目標価格 < エントリー価格）

【購入（空売り）株数の決定】
- 推奨ポジションが決まった場合、利用可能資金とリスク許容度（例：1トレードあたりの損失を資金の1-2%に抑える）を考慮して、適切な株数を100株単位で決定してください。
- リスク（エントリー価格とストップロスの差）から株数を計算し、最大可能株数を超えないようにしてください。

回答は以下の形式で格納してください:
```json
{{
  "position": "long" or "short" or "hold",
  "should_enter": true (positionがlong or shortの場合) or false (positionがholdの場合),
  "confidence": 0-100,
  "reasoning": "判断理由を簡潔に説明（ロング・ショート両方の評価を含む）",
  ...
  "rule": {{
    ...
    "quantity": "推奨購入/空売り株数（最小:100株、以降100株単位の整数）" or "NG"
  }}
}}
```
"""
