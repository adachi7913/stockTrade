...
class ApiHandler:
    ...
    def get_prompt(self):
        ...
        **日本語のみ**で回答してください。
        ...
        【バックテスト戦略の説明】
        提供されるバックテスト結果は、以下の**5つ**の戦略を使用して生成されています。各戦略の特徴を理解し、現在の市場状況に最も適した戦略を判断してください。
        ...
        4. 移動平均線クロス+RSI戦略（ma_rsi）:
           - 使用指標: 短期SMA(5), 中期SMA(20), 長期SMA(75), RSI(14), ATR(14)
           - エントリー条件: 長期SMA(75)が上昇中 かつ (短期SMA(5)が中期SMA(20)をゴールデンクロス **または** RSI(14)が30をゴールデンクロス)
           - 決済条件: 短期SMA(5)が中期SMA(20)をデッドクロス **または** RSI(14)が70をデッドクロス **または** エントリー価格 - (1.5 * ATR)を下回る **または** エントリーから5営業日経過
           - 特徴: 長期トレンドに沿いつつ、短期的な移動平均線のクロスまたはRSIの反発をエントリーシグナルとする戦略。

        5. ボリンジャーバンド+長期トレンド戦略（bb_macd）:
           - 使用指標: ボリンジャーバンド(20, 2σ), 長期SMA(75), ATR(14), MACD(12, 26, 9)（決済で使用）
           - エントリー条件: 長期SMA(75)が上昇中 かつ 株価がボリンジャーバンド下限(2σ)にタッチまたは下回る
           - 決済条件: 株価がボリンジャーバンド上限(2σ)にタッチ **または** MACDラインがシグナルラインをデッドクロス **または** エントリー価格 - (1.5 * ATR)を下回る **または** エントリーから5営業日経過
           - 特徴: 長期トレンドに沿いつつ、ボリンジャーバンド下限での反発を狙う戦略。MACDは決済シグナルの一つとして利用。
