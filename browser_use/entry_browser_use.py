...
9.少し下にスクロールする
10.価格が指値・条件なしになっていることを確認
11.IFD1の右側に価格があることを確認し、さらにその右側のテキストボックスに{entry_data['entry_price']}と入力。
12.IFD1の期間は、期間指定を選択
13.右側の日付は{entry_data['date']}に最も近い日付を選択
14.預かり区分はNISA預かりに変更する
15.IFD2のOCO1は「条件なし」の{entry_data['target_price']}になるように入力
16.OCO2の逆指値は{entry_data['stop_loss']}と入力
17.OCO2の成行を選択
18.OCO2の期間は、期間指定を選択
19.OCO2の日付は{entry_data['date']}に最も近い日付を選択
20.取引パスワードに{self._get_tread_password()}を入力
21.「注文確認画面を省略」にチェックを入れる
22.注文発注ボタンをクリック
23.注文が完了したら、注文の約定状況を確認し、以下のいずれかの結果を返してください：
